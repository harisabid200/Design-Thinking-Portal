...
/**
 * UserPersonaEditor - For creating user persona profiles
 */
const UserPersonaEditor = ({ content, onChange, projectTitle }) => {
  return (
    <div className="space-y-6">
      <div className="p-4 bg-purple-900/30 rounded-lg border border-purple-500/30">
        <h3 className="font-medium text-purple-300 mb-1">👤 User Persona Guide</h3>
        <p className="text-sm text-slate-300">
          Based on your research for <strong>{projectTitle}</strong>, create a representative 
          user profile that captures the key characteristics of your target users.
        </p>
      </div>
      ...
      <div className="p-4 bg-slate-800 rounded-xl border border-slate-700 space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-slate-400 mb-1">Key Characteristics</label>
          <textarea
            value={content.characteristics || ''}
            onChange={(e) => onChange({ ...content, characteristics: e.target.value })}
            className="w-full h-20 px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white placeholder-slate-400 focus:border-purple-500 focus:outline-none resize-none"
            placeholder="Describe their typical behaviors, preferences, and context"
          />
        </div>
      </div>
    </div>
